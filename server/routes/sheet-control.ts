...
router.post("/sources", zValidator("json", createAgentSheetSourceSchema), async (c) => {
  ...
  try {
    const source = await createAgentSheetSource(c.env.DB, {
      agentId: body.agent_id,
      sheetUrl: body.sheet_url,
      sheetTabName: body.sheet_tab_name ?? null,
    ...
    return c.json({ source, message: "Agent sheet source created." }, 201);
  } catch (error) {
